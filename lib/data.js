...
const getCoursesPerTerm = async (status, term) => {
  ...
  const termID = getTerm(term);

  // Class Form
  await page.select("#term_dropdown", termID);
  await page.select("#reg_status", status.trim());

  const SUBMIT = ".btn.btn-lg.btn-primary.btn-block";
  await page.click(SUBMIT);
};

getCoursesPerTerm("all", "2017 Fall Quarter");

module.exports.getCourse = async course => {
  /*
   * Course Setup ------------------------------------------------------
  */
  const courseSubject = getSubject(course);
