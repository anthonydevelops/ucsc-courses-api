...
const getCourse = async (course, term) => {
  /*
   * Course Setup ------------------------------------------------------
   */
    ...
    const COURSE_INFO = await page.evaluate(course => {
      try {
        ...
        return {
          ...
          sections: headers
            .map((i, elem) => {
              if (i.innerText === "Associated Discussion Sections or Labs") {
                return sectionInfo;
              }
            })
            .filter(n => n != null)[0]
  ...
  if (LINK.length > 0) {
    for (let i = 0; i < LINK.length; i++) {
      ...
      const CLASSES = await infoList(course);
  ...
  const result = await COURSE_DATA.filter(n => n != null || n != undefined);
